...
public class ContentEngagementScheduler {
    ...
    public void tick() {
        ...
        if (activeCampaigns.isEmpty()) {
            log.debug("Нет активных кампаний для обновления");
            return;
        }
        int incremented = 0;
        int completed = 0;
        ...
        for (AdvertisingCampaign campaign : activeCampaigns) {
            for (Content content : campaign.getContent()) {
                if (content == null) {
                    continue;
                }
        ...
        log.info("Engagement tick completed: incremented={}, completed={}", incremented, completed);
    }
}
